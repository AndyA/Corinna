...
Corinna::ComplexType->mk_accessors(qw());
...
B<Corinna::ComplexType> - Ancestor of all complex classes generated by L<Corinna>
...
First, you need to generate your classes using L<Corinna>. See the documentation of L<Corinna> for more examples of this.
L<Corinna> will then generate classes corresponding to the global elements and type definitions (complex and simple) in your W3C schema.
...
B<Corinna::ComplexType> is an B<abstract> ancestor of all complex classes (including those corresponding to global elements) generated 
...
B<Corinna::ComplexType> defines several methods that serve mainly to achieve the XML binding of generated classes that inherit from it.
...
In fact, B<Corinna::ComplexType> contains (actually I<inherits from> L<Corinna::Type>) a class data accessor called L</XmlSchemaType()> with the help of L<Class::Data::Inheritable>. This
...
does not contain any information and this is why B<Corinna::ComplexType> remains abstract. 
...
Generated classes for simple types do not descend from this class. Instead, they descend from L<Corinna::SimpleType> (although both have a common ancestor 
called L<Corinna::Type>).
  
.

=head1 METHODS

B<WARNING 1> : Normally, you never call the methods defined by B<Corinna::ComplexType> on this class itself. Instead,
...
conventions given by L<Corinna>, you should be OK in any case. 
See L<Corinna/SUGGESTED NAMING CONVENTIONS FOR XML TYPES, ELEMENTS AND ATTRIBUTES IN W3C SCHEMAS> for more info.
...
B<Corinna::ComplexType> does not define accessors on itself, but the generated classes
...
that descends eventually from L<Corinna::SimpleType>. This may be a builtin class defined by L<Corinna::Builtin> or
...
But be warned that the accessor does not change your SCALAR or object into an object of correct class. B<Corinna> is 
...
of a class that eventually descends from B<Corinna::ComplexType>. This will typically be a generated class that derives from
...
accessor will return a L<Corinna::NodeArray> object (which is just a blessed array with some overloading functionality) that will 
...
class that descends eventually from B<Corinna::ComplexType> or L<Corinna::SimpleType> depending on whether the element has simple
...
to use a L<Corinna::NodeArray> object just as it is returned by the accessor (GETTER). But you don't have to. You can use a plain array
...
See also  L<Corinna::Type>, L<Corinna::SimpleType>, L<Corinna>

And if you are more curious, see L<Corinna::Type> (the ancestor of this class), 
L<Corinna::Schema::ComplexType> (meta type information from your B<W3C schema>)
