...
B<Corinna::Type> - Ancestor of L<Corinna::ComplexType> and L<Corinna::SimpleType>.
...
B<Corinna::Type> is an B<abstract> ancestor of L<Corinna::ComplexType> and L<Corinna::SimpleType> and 
therefore indirectly of all simple and complex classes generated by L<Corinna> which is a Perl code 
generator from W3C XSD schemas. For an introduction, please refer to the
documentation of L<Corinna>.

B<Corinna::SimpleType> defines a class data accessor called L</XmlSchemaType()> 
...
The new() constructor method instantiates a new B<Corinna::Type> object. It is inheritable, and indeed inherited,
by the generated decsendant classes. Normally, you do not call the B<new> method on B<Corinna::Type>. You rather
...
As a side note, notice that child elements that can I<potentially> have multiple values will always be put in a L<Corinna::NodeArray> object when
...
L<Corinna::NodeArray> objects have some magical properties that occur thanks to hash access overloading and the AUTOLOAD method that enable them to be used 
as an array of fields or as if it were a reference to the first node in the array. See L<Corinna::NodeArray> for more details.

When writing to or validating XML, L<Corinna> is quite forgiving. Non-array (singleton) values are accepted as if they were an array having only one item inside.
...
B<Corinna::ComplexType> or L<Corinna::SimpleType>.
...
B<Corinna::Type> defines (thanks to L<Class::Data::Inheritable>) 
...
This data is of class L<Corinna::Schema::ComplexType> or L<Corinna::Schema::SimpleType>. 
...
which should perform extra checks. For B<Corinna::Type> this always returns TRUE, but some builtin types 
...
For B<Corinna::Type>, this method simple returns TRUE(1).

This method may be, and is indeed, overriden by subclasses. Several builtin classes like
like L<Corinna::Builtin::date> and L<Corinna::Builtin::dateTime> override this method.
